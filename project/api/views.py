...
def add_dep(request):
    if request.method=="POST":
        ...
        data = DepartmentModel.objects.filter(dep_name=dep_name)
        if data:
            ...
            DepartmentModel.objects.create(dep_name=dep_name,
...
def add_stu(request):
    if request.method=='POST':
        ...
        # data = StudentModel.objects.filter(Q(stu_department=stu_department) & Q(stu_email=stu_email))
        data = StudentModel.objects.filter(stu_email=stu_email)
        if data:
            if data.stu_department_id:
                ...
                data = DepartmentModel.objects.get(id=stu_department)
